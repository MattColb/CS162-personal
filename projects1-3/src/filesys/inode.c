...
struct cache_block* clock_find_victim() {
...
struct cached_block* get_cached_block(block_sector_t sector) {
  ...
  if (cb == NULL) {
    ...
    bool flush = false;
    block_sector_t old_sec = cb->sector;
    ...
    evict_and_replace(cb, old_sec, flush);
    lock_release(&cb->lock);
  } else {
    lock_release(&buffer_cache->lock);
  }
...
void cache_read(block_sector_t sec, void* buff, off_t offset, size_t chunk_size) {
  while (true) {
    struct cached_block* cb = get_cached_block(sec);
    lock_acquire(&cb->lock);
    if (cb->sector == sec) {
      memcpy(cb->data + offset, buff, chunk_size);
      lock_release(&cb->lock);
      return;
    }
    lock_release(&cb->lock);
...
void cache_write(block_sector_t sec, void* buff, off_t offset, size_t chunk_size) {
  while (true) {
    struct cached_block* cb = get_cached_block(sec);
    lock_acquire(&cb->lock);
    if (cb->sector == sec) {
...
struct inode_disk {
  ...
  block_sector_t doubly_indirect; /* First data sector. */
  off_t length;         /* File size in bytes. */
  unsigned magic;       /* Magic number. */
  bool is_dir;
  uint32_t unused[111]; /* Padding. */
};
...
static block_sector_t byte_to_sector(const struct inode* inode, off_t pos) {
  ...
  if (pos < inode_lentgh(inode)){
    ...
    // INDIRECT
      ...
      sec = block_buffer[sector_index - DIRECT_BLOCKS];
    }
    ...
    // DOUBLY
      ...
      sec = block_buffer[sector_in_single];
    }
  ...
  if (freed < blocks_to_free && disk_inode->doubly_indirect != 0) {
    ...
    for (int i =0; i < BLOCK_SECTOR_SIZE / sizeof(block_sector_t) && freed < blocks_to_free; i++) {
      if (doubly_indirect[i] != 0) {
        ...
        for (int j =0; j < BLOCK_SECTOR_SIZE / sizeof(block_sector_t) && freed < blocks_to_free; j++) {
          if (indirect[j] != 0) {
